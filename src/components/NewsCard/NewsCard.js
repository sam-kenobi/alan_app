...
const NewsCard = ({ article: { description, publishedAt, source, title, url, urlToImage }, i,curArticle }) => {
  ...
  useEffect(() => {
    if (i === curArticle && elRefs[curArticle]) {
      scrollToRef(elRefs[curArticle]);
    }
  }, [i, curArticle, elRefs]);

  console.log(i,curArticle);
  return (
    <Card ref={elRefs[i]} className={ curArticle === i ? classes.activeCard : classes.card}>
